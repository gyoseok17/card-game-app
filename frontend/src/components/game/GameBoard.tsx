...
export default function GameBoard({ sendAction, sendChat }: Props) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-800 to-green-950 flex flex-col relative">
      {/* 상단: 상대 플레이어 */}
      ...
      <button
        ...
        className="fixed bottom-20 right-4 bg-green-500 text-white w-12 h-12 rounded-full shadow-lg hover:bg-green-400 transition z-40 text-lg"
      >
        {showChat ? '\u2715' : '\uD83D\uDCAC'}
      </button>

      {showChat && <GameChat sendChat={sendChat} />}
